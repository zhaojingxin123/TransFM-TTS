...
def save_to_folder(filename: str, output: dict, folder: str,emo_name):
    # 将folder字符串转换为Path对象
    folder = Path(folder)
    ...
    plot_path=folder / f"{filename}_{emo_name}.png"
    # 打印图像保存路径
    # print(plot_path)
    ...
    # 
    np.save(folder / f"{filename}_{emo_name}", output["mel"].cpu().numpy())
    
    sf.write(folder / f"{filename}_{emo_name}.wav", output["waveform"], 22050, "PCM_24")
    
    return folder.resolve() / f"{filename}_{emo_name}.wav"
...
def cli():
    ...
    if len(texts) == 1 or not args.batched:
        # ---
        unbatched_synthesis(args, device, model, vocoder, denoiser, texts, spk,emo_path)
    else:
...
# 不批量的合成
def unbatched_synthesis(args, device, model, vocoder, denoiser, texts, spk,emo_path):
    total_rtf = []
    total_rtf_w = []
    for i, text in enumerate(texts):
        ...
        location = save_to_folder(base_name, output, args.output_folder,emo_name)
