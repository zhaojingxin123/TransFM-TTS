...
class TextMelDataset(torch.utils.data.Dataset):
    ...
    def get_datapoint(self, filepath_and_text):
        if self.n_spks > 1:
            filepath, spk, text = (
                filepath_and_text[1],
                int(filepath_and_text[0]),
                filepath_and_text[4],
            )
        else:
            filepath, text = filepath_and_text[1], filepath_and_text[4]
            spk = None
        ...
        return {"x": text, "y": mel, "spk": spk, "filepath": filepath, "x_text": text, "durations": durations}
