...
def parse_filelist(filelist_path, split_char="|"):
    with open(filelist_path, encoding="utf-8") as f:
      ...
      for line in f:
        ...
        # {speaker_map[speaker]}|{wave_file}|{spec_file}|{emo_path}|{phone_items_str}|{text}|{phone_ID}
        ...
        #   print(type(phone_ID[1]))
          filepaths_and_text.append([aa[1],aa[5],aa[2],aa[4],aa[0],aa[6],aa[3]])
...
class TextMelDataset(torch.utils.data.Dataset):
    ...
    def get_datapoint(self, filepath_and_text):
        ...
        emo = torch.load(filepath_and_text[6])
        # print(type(emo))
        # print("emo:",emo.shape)
        ##---------------------------------------------------------------##
...
class TextMelBatchCollate:
    ...
    def __call__(self, batch):
        ...
        # dtype=torch.float32
        # dtype=torch.long是长整型
        x = torch.zeros((B, x_max_length), dtype=torch.long)
        emos=torch.zeros((B, 1024), dtype=torch.float32)
        ...
        # emos = torch.tensor(emos, dtype=torch.long) if self.n_spks > 1 else None
