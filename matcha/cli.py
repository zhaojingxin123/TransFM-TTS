...
# 添加父目录到 sys.path
sys.path.append('/home/zjx/Matcha-TTS/vits_chinese-bert_vits_aishell3/')
# import vits_pinyin
# 对于__init__里的函数是直接导入/home/zjx/Matcha-TTS/vits_chinese-bert_vits_aishell3/text
sys.path.append('/home/zjx/Matcha-TTS/vits_chinese-bert_vits_aishell3/text/')
# import symbols
...
MULTISPEAKER_MODEL = {
    "matcha_vctk": {"vocoder": "hifigan_univ_v1", "speaking_rate": 0.85, "spk": 10, "spk_range": (0, 107)}
}
...
def process_text(i: int, text: str, device: torch.device):
    ...
    x = torch.tensor(
        intersperse(text_to_sequence(text, ["chinese_cleaners"])[0], 0),
        dtype=torch.long,
        device=device,
    )[None]
...
# 处理文本，这里需要可以改成aishell的
#--------------aishell的---------------------#
def process_text_aishell(i: int, text: str, device: torch.device):
    print(f"[{i}] - Input text: {text}")

    tts_front=vits_pinyin.VITS_PinYin("./bert", device)
    n = 0
    while (True):
        try:
            item = text.strip()
        ...
        if (item == None or item == ""):
            break
        n = n + 1
        phonemes = tts_front.chinese_to_phonemes_foraishell3_noembeding(item)
        aishell_phonemes_ids = symbols.cleaned_text_to_sequence(phonemes)
        print(f'aishell_phonemes_ids:{aishell_phonemes_ids}')
        break


    x = torch.tensor(
        intersperse(aishell_phonemes_ids, 0),
        dtype=torch.long,
        device=device,
    )[None]
    ...
    x_phones = symbols.sequence_to_text(x.squeeze(0).tolist())
    #这里有问题
    print(f"[{i}] - Phonetised text: {x_phones}")
    print(f"[{i}] - Phonetised text: {x_phones.split(' ')}")
