...
from matcha.cli_aishell_noemo import (
    MATCHA_URLS,
    VOCODER_URLS,
...
# 写成可以用的
args.model="transFM"
ckpth="/home/zjx/Matcha-TTS/logs/train/aishell3_noemo/runs/2025-02-06_16-26-28/checkpoints/checkpoint_epoch=099.ckpt"
model = load_matcha(args.model, ckpth, device)
...
# def jiahanshu(text, n_timesteps, mel_temp, length_scale, spk):
#     phones, text, text_lengths = process_text_gradio(text)
#     # phones, audio, mel_spectrogram = synthesise_mel(text, text_lengths, n_timesteps, mel_temp, length_scale, spk)
...
def main():
    description = """# 😊TransFm-TTS: A fast TTS architecture with conditional flow matching
    
    We propose 😊TransFm-TTS, a new approach to non-autoregressive neural TTS, that uses conditional flow matching (similar to rectified flows) to speed up ODE-based speech synthesis. Our method:
    ...
    * Is very fast to synthesise from
    ...
    with gr.Blocks(title="😊 TransFM-TTS: A fast TTS architecture with conditional flow matching") as demo:
        ...
        with gr.Box():
            ...
            with gr.Row():
                text = gr.Textbox(value="", lines=2, label="Text to synthesise 待合成文本", scale=3)
                spk_slider = gr.Slider(
                    minimum=0, maximum=107, step=1, value=args.spk, label="Speaker ID 说话人 ID", interactive=True, scale=1
                )
            ...
            with gr.Row():
                n_timesteps = gr.Slider(
                    label="Number of ODE steps 合成步数",
                    minimum=1,
                    maximum=100,
                    step=1,
                    value=10,
                ...
                length_scale = gr.Slider(
                    label="Length scale (Speaking rate) 说话速度",
                    minimum=0.5,
                    maximum=1.5,
                    step=0.05,
                    value=1.0,
                ...
                mel_temp = gr.Slider(
                    label="Sampling temperature 采样方差",
                    minimum=0.00,
                    maximum=2.001,
                    step=0.16675,
                    value=0.667,
                ...
                synth_btn = gr.Button("Synthesise 合成")
        ...
        model_type.change(lambda x: gr.update(interactive=False), inputs=[synth_btn], outputs=[synth_btn]).then(
            ...
            outputs=[text, synth_btn, spk_slider, length_scale],
        )
