...
from matcha.models.matcha_tts_noemo_mat import MatchaTTS
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                # 检查是否有足够的元素
                if len(elements) >= 6:
                    fifth_element = elements[5]  # 索引 4 对应第五个元素
                    result_array.append(fifth_element)
                else:
                    print(f"警告：行内容不足6个元素，跳过该行 - {line.strip()}")
...
def process_text_aishell_TW(i: int, text: str, device: torch.device):
    ...
    return {"x_orig": text, "x": x, "x_lengths":x_lengths, "x_phones": clean_text,"text_length":text_length}
...
def cli():
    ...
    parser.add_argument(
        "--batch_size", type=int, default=2, help="Batch size only useful when --batched (default: 32)"
    )
    ...
    # emo.to(device)
    # /home/zjx/Matcha-TTS/data/aishell3/valid_match_taiwan_notensor.txt
    # /home/zjx/Matcha-TTS/data/zhugehuafenhaodewenben.txt
    if args.batched:
        # texts=read_and_extract('/home/zjx/Matcha-TTS/data/aishell3/valid_match_taiwan_notensor_50.txt')
        # texts=read_and_extract('/home/zjx/Matcha-TTS/data/aishell3/valid_match_taiwan_notensor.txt')
        texts=read_and_extract('/home/zjx/Matcha-TTS/data/zhugehuafenhaodewenben_fuzhi.txt')
...
def batched_collate_fn(batch):
    ...
    for b in batch:
        ...
        text_lengths.append(b["text_length"])
        print("text",b["x_orig"])
        print("text_length",b["text_length"])
    x = torch.nn.utils.rnn.pad_sequence(x, batch_first=True)
    x_lengths = torch.concat(x_lengths, dim=0)
    return {"x": x, "x_lengths": x_lengths,"text_orig_length": text_lengths}
...
def batched_synthesis(args, device, model, vocoder, denoiser, texts, spk):
    ...
    num_of_juzi=0
    # filename= "/home/zjx/Matcha-TTS/data/mat-1坐标数据.txt"
    filename= "/home/zjx/Matcha-TTS/data/mat-20坐标数据-1.txt"
    data_csv=[]
    with open(filename, 'w', encoding='utf-8') as file:
        for i, batch in enumerate(dataloader):
            ...
            b = batch["x"].shape[0]
            text_length=batch["text_orig_length"][0]
            ...
            #############################
            print(f"总耗时：{t}s,{text_length}个字符")
            line = f"{text_length}|{t}\n"
            data_csv.append({"X坐标": text_length, "Y坐标": t})  # 将每一对 (x, y) 数据存入字典，并添加到列表中
            file.write(line)  
            ...
            for j in range(output["mel"].shape[0]):
                num_of_juzi+=1
                base_name = f"{num_of_juzi}utterance_{j:03d}_speaker_{args.spk:03d}" if args.spk is not None else f"utterance_{j:03d}"
    ...
    # 写入 CSV 文件
    csv_filename = "/home/zjx/Matcha-TTS/data/mat-20坐标数据.csv"
